// ══════════════════════════════════════════════════════════════
    ...
    async function openStudy(book, chapter, verse, text){
        ...
        const _cur=CURATED[_sAccents(sbStudyData.ref).toLowerCase()];
        document.querySelectorAll('.sp-tab').forEach(t=>t.classList.remove('active'));
        const first=document.querySelector('.sp-tab[data-sp="ensenanza"]'); if(first) first.classList.add('active');
        ['ensenanza','contexto','conexiones','palabra','cristo','voces','versiones','reflexion'].forEach(sbRenderStudy);
        // Pestañas nuevas: solo se muestran en versículos que ya tienen ese contenido.
        const _tC=document.querySelector('.sp-tab[data-sp="cristo"]'); if(_tC) _tC.style.display=(_cur&&_cur.christ)?'':'none';
        const _tV=document.querySelector('.sp-tab[data-sp="voces"]'); if(_tV) _tV.style.display=(_cur&&_cur.voices)?'':'none';
    ...
    function sbRenderStudy(tab){
        ...
        if(tab==='ensenanza'){
            ...
            if(cur && cur.words){
                pane.innerHTML='<p class="sp-hint">Estudio de palabras clave en su idioma original (hebreo · arameo · griego).</p>'+
                    cur.words.map(w=>{
                        const lang=w.lang?'<span class="sp-word-lang lang-'+_sAccents(w.lang).toLowerCase()+'">'+sbEsc(w.lang)+'</span>':'';
                        const pron=w.pron?'<span class="sp-word-pron">/'+sbEsc(w.pron)+'/</span>':'';
                        const tl=w.translit?'<div class="sp-word-tlrow"><span class="sp-word-tl">'+sbEsc(w.translit)+'</span></div>':'';
                        const meta=[];
                        if(w.root) meta.push('<div class="sp-word-meta"><span>Raíz</span>'+sbEsc(w.root)+'</div>');
                        if(w.freq) meta.push('<div class="sp-word-meta"><span>Aparece</span>'+sbEsc(w.freq)+'</div>');
                        const why=w.why?'<div class="sp-word-why"><span class="sp-word-why-l">✦ Por qué importa aquí</span>'+sbEsc(w.why)+'</div>':'';
                        return '<div class="sp-card sp-word">'+
                            '<div class="sp-word-top"><div class="sp-word-head">'+lang+'<span class="sp-word-es">'+sbEsc(w.es)+'</span></div>'+(w.strong?'<span class="sp-word-st">'+sbEsc(w.strong)+'</span>':'')+'</div>'+
                            '<div class="sp-word-or2">'+sbEsc(w.or)+pron+'</div>'+tl+
                            '<p class="sp-card-p">'+sbEsc(w.meaning)+'</p>'+
                            (meta.length?'<div class="sp-word-metas">'+meta.join('')+'</div>':'')+why+'</div>';
                    }).join('');
            } else { pane.innerHTML=sbSoon('Palabra original (hebreo/arameo/griego) — próximamente'); }
        }
        else if(tab==='cristo'){
            if(cur && cur.christ){
                const c=cur.christ;
                pane.innerHTML=(c.lead?'<div class="sp-bigidea"><span class="sp-bigidea-l">✝ Cristo en el texto</span><p>'+sbEsc(c.lead)+'</p></div>':'')+
                    '<div class="sp-teach">'+_paras(c.paras).map(p=>'<p>'+sbEsc(p)+'</p>').join('')+'</div>';
            } else { pane.innerHTML=sbSoon('Cómo este pasaje apunta a Cristo — próximamente'); }
        }
        else if(tab==='voces'){
            if(cur && cur.voices){
                pane.innerHTML='<p class="sp-hint">Lo que grandes maestros de la historia vieron en este versículo.</p>'+
                    cur.voices.map(v=>'<div class="sp-voice"><div class="sp-voice-q">“'+sbEsc(v.quote)+'”</div><div class="sp-voice-by"><span class="sp-voice-a">'+sbEsc(v.author)+'</span>'+(v.era?'<span class="sp-voice-e">'+sbEsc(v.era)+'</span>':'')+'</div></div>').join('');
            } else { pane.innerHTML=sbSoon('Voces de la historia — próximamente'); }
        }
