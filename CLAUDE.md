...
| `backend/emails/` | Plantillas de correo (HTML + texto), cada una **una sola vez por uid**: `premium-bienvenida.js` al activarse Premium (Stripe, cupón o cortesía; marca KV `welcome:<uid>`) y `cuenta-bienvenida.js` la primera vez que entra un usuario NO premium, desde `/api/me` (qué tiene gratis + prueba de 7 días; marca KV `hello:<uid>`). `base.js` tiene la paleta y el maquetado que comparten; el contenido nunca se duplica. Previsualizar con `node backend/tools/render-welcome.mjs`, o `POST /api/admin/send-welcome` con `{preview:true}` para ver el HTML **vivo en producción**. Reenvío a mano: mismo endpoint con `{email, uid?, tipo?: 'premium'|'cuenta', force?, asunto?}` (`asunto` evita que Gmail agrupe la copia nueva bajo la vieja). Envío a los Premium anteriores: `POST /api/admin/welcome-backfill` (`dry_run` por defecto). Todos con header `X-Admin-Secret`. Todo el correo del worker sale por `brevoSend()`, que pone `Reply-To: contacto@sonidodevida.com` (el remitente `noreply@` no lee a nadie: **no escribir "responde a este correo"** en las plantillas). |
